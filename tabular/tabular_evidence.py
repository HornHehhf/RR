...
def get_selected_candidates(test_path, output_path):
    ...
    for idx, test_case in enumerate(test_data):
        ...
        premise = str(test_case['premise']).strip()
        nlp_premise = nlp(premise).sents
        premise_sents = []
        for sent in nlp_premise:
            sent = str(sent).strip()
            if len(sent) != 0:
                premise_sents.append(sent)
        relation_sents = [relation.strip() for relation in test_case['relations']]
        knowledge_sents = []
        # knowledge_sents.extend(premise_sents)
        knowledge_sents.extend(relation_sents)
        ...
        for prediction in predictions:
            ...
            for sent in nlp_text:
                sent = str(sent)
                sent_relations.append((sent, knowledge_sents))
            candidate_predictions.append(sent_relations)
        selected_candidates = []
...
if __name__ == '__main__':
    ...
    self_consistency_path = dir_path + 'GPT-3/binary_dev_self_consistency_gpt3.json'
    self_consistency_selected_path = \
        dir_path + 'GPT-3/binary_dev_self_consistency_gpt3_selected_candidates.json'
    self_consistency_entailment_path = dir_path + 'GPT-3/binary_dev_self_consistency_gpt3_entailment.json'
    time_start = time.time()
    get_selected_candidates(self_consistency_path, self_consistency_selected_path)
